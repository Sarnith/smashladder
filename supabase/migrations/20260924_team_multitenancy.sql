...
create or replace function public.save_current_game_score(p_team_id uuid, p_court_index integer, p_game_index integer, p_expected_score_1 integer, p_expected_score_2 integer, p_score_1 integer, p_score_2 integer)
returns void language plpgsql security definer set search_path = public as $$
declare v_data jsonb; v_current_score_1 integer; v_current_score_2 integer;
begin
  ...
  if v_current_score_1 is distinct from p_expected_score_1 or v_current_score_2 is distinct from p_expected_score_2 then
    raise exception 'Score conflict: this game was updated by another scorer. Reloaded the latest score.' using errcode = 'P0001';
  end if;
